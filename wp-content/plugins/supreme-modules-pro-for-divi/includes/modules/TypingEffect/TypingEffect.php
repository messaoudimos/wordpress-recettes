...
class DSM_TypingEffect extends ET_Builder_Module {
	...
	public function render( $attrs, $content, $render_slug ) {
		...
		if ( '' !== $before_typing_effect ) {
			...
			$before_typing_effect = $multi_view->render_element(
				array(
					'tag'     => 'span',
					'content' => '{{before_typing_effect}}',
					'attrs'   => array(
						'class' => 'dsm-before-typing-effect',
					),
				)
			);
		}
		...
		if ( '' !== $after_typing_effect ) {
			...
			$after_typing_effect = $multi_view->render_element(
				array(
					'tag'     => 'span',
					'content' => '{{after_typing_effect}}',
					'attrs'   => array(
						'class' => 'dsm-after-typing-effect',
					),
				)
			);
		}
		...
		if ( '' !== $typing_effect ) {
			$typing_effect = sprintf(
				'<%1$s class="dsm-typing-effect et_pb_module_header" aria-label="%9$s">%7$s %14$s<span class="dsm-typing-wrapper"><span class="dsm-typing-strings">%9$s</span><span class="dsm-typing" aria-hidden="true" data-dsm-typing-cursor="%13$s" data-dsm-typing-strings="%2$s"%3$s%4$s%5$s%6$s%10$s%11$s%12$s></span></span>%15$s%8$s</%1$s>',
